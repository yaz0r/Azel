...
void sCutsceneTask::Init(sCutsceneTask* pThis, s32 arg)
{
...
void sCutsceneTask::cutsceneDrawSub0(sCutsceneTask* pThis, s32 r5)
{
    s32 iVar3 = 0x400;
...
void sCutsceneTask::Draw(sCutsceneTask* pThis)
{

    if ((g_fadeControls.m0_fade0.m20_stopped != 0) &&
        (((readKeyboardToggle(KEY_CODE_F12) != 0 && ((gCutsceneFlags & 4) == 0)) ||
...
// --- Town cutscene script functions ---

s32 townCreateCutscene(s32 arg)
{
    if (npcData0.mF0 != 0) {
...
s32 townIsCutsceneDone()
{
    if ((gCutsceneTask != nullptr) && (npcData0.mF0 == 0)) {
        s32 state = scriptFunction_605861eSub0Sub0(gCutsceneTask->m0);
        if (state != 5 && state != -1) {
            return 0;
        }
    }
...
s32 townDeleteCutscene()
{
    if (gCutsceneTask != nullptr) {
        npcData0.mF4 = 0;
        updateStreamingFileReadSub0(gCutsceneTask->m0);
        sCutsceneTask* temp = gCutsceneTask;
        gCutsceneTask = nullptr;
...
s32 townGetCutsceneHalfPosition()
{
    if (gCutsceneTask != nullptr && npcData0.mF0 == 0) {
        s32 pos = getPositionInEDK(gCutsceneTask->m0);
